...
vuelos$STD_DiaSemana <- weekdays(vuelos$STD_fecha)
vuelos$STA_DiaSemana <- weekdays(vuelos$STA_fecha)
dias_semana <- c("lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo")
vuelos$STD_DiaSemana <- match(vuelos$STD_DiaSemana, dias_semana)
vuelos$STA_DiaSemana <- match(vuelos$STA_DiaSemana, dias_semana)
```
...
paste del departure station y destination type
...
```{r}
df_alt$Destination_Type  <- NULL
df_alt$Origin_Type<- NULL
df_alt$STA_fecha<- NULL
df_alt$STA_hora<- NULL
df_alt$Longitud_viaje<- NULL
```
...
df_alt$Flight_ID <- NULL
df_alt$ProductType <- NULL
df_alt$Type <- NULL
df_alt$STA_fecha <- NULL
df_alt$STD_fecha <- NULL
df_alt$STD <- NULL
df_alt$STA <- NULL
df_alt$STA_hora <- NULL
df_alt$STD_Año <- NULL
df_alt$DepartureStation <- NULL
df_alt$ArrivalStation <- NULL
...
```{r}
model_ols <- lm(Quantity ~ Passengers+Capacity+STD_Mes+STD_Dia_del_mes+ventas_totales_producto_hora+ventas_promedio_producto_hora+ventas_totales_producto_ruta+ventas_promedio_producto_ruta+ruta, data = train)
summary(model_ols)
...
# RMSE
# RMSE sin aplicar exp a las predicciones
...
rf_model <- randomForest(
  Quantity ~ Passengers+Capacity+STD_Mes+STD_Dia_del_mes+ventas_totales_producto_hora+ventas_promedio_producto_hora+ventas_totales_producto_ruta+ventas_promedio_producto_ruta, 
  data = train,

...
```{r}
aeroventas <- function(hora, ruta, pasajeros, capacity, mes, dia_sem, Dmes, Dfmin) {
  ...
  #### TRATAR LOS NULOS ####  
    ...
    diferencia_minutos = Dfmin
    )
    #newdata <- sapply(newdata, as.numeric)
    #matriz_datos <- xgb.DMatrix(data = as.matrix(newdata), missing = NA)

    # Hacer la predicción
    prediccion <- predict(model_ols, newdata)
    #prediccion <- predict(xgb_model, matriz_datos) 
...
aa = aeroventas(hora, ruta, pasajeros, capacity, mes, dia_sem, Dmes, Dfmin)
print(aa)
```

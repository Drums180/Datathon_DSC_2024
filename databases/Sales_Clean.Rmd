...
## Database
```{r}
ventas <- read.csv("Sales TEC_Valid.csv")
```

## Unicos por Columna de Text
```{r}
# Función para obtener valores únicos por columna
unicos_por_columna <- function(data) {
  ...
  # Iterar sobre todas las columnas del conjunto de datos
  for (col in colnames(data)) {
    # Obtener valores únicos para la columna actual
    valores <- unique(data[[col]])
    
    # Almacenar la longitud de los valores únicos
    longitud_valores <- length(valores)
    
    # Agregar resultados a la tabla
    resultados <- rbind(resultados, data.frame(columna = col,
                                               valores_unicos = longitud_valores))
  }
...
resultado <- unicos_por_columna(ventas)
...
write.csv(ventas, "Sales_Clean.csv")
```
